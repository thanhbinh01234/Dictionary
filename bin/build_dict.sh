...
while getopts c:e:s:t:v: opt; do
    case $opt in
    c)
    ...
    s)
        ...
        if [ $NORMALIZE_SETTING -eq 0 ]; then
...
# The make_body.pl creates $OBJECTS_DIR/dict.body and $OBJECTS_DIR/dict.offsets
...
if [ $do_add_supplementary_key -gt 0 ]; then
...
if [ $preserve_unused_ref_id_in_reference_index -gt 0 ]; then
...
plistutil --infile "$OBJECTS_DIR"/Info.plist
...
plistutil --infile "$OBJECTS_DIR"/dict.dictionary/Contents/Info.plist

# Copy other files.
echo "- Copying CSS."
cp -f "$CSS_NAME" "$OBJECTS_DIR"/dict.dictionary/"$CONTENTS_DATA_PATH"/DefaultStyle.css || error "Error."
if [ -d "$OTHER_RSRC_DIR" ] && [ -n "$(ls $OTHER_RSRC_DIR)" ]; then
